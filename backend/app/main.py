...
def atualizar_cliente(cliente_id: int, cliente_update: ClienteCreate, session: Session = Depends(get_session)):
    cliente = session.get(Cliente, cliente_id)
    if not cliente:
        raise HTTPException(status_code=404, detail="Cliente não encontrado")
...
def atualizar_agendamento(agendamento_id: int, agendamento_data: AgendamentoCreate, session: Session = Depends(get_session)):
    agendamento = session.get(Agendamento, agendamento_id)
    if not agendamento:
        raise HTTPException(status_code=404, detail="Agendamento não encontrado")
...
def listar_agendamentos_cliente(cliente_id: int, session: Session = Depends(get_session)):
    # Verificar se o cliente existe
    cliente = session.get(Cliente, cliente_id)
    if not cliente:
        raise HTTPException(status_code=404, detail="Cliente não encontrado")
...
def listar_agendamentos_data(data_consulta: date, session: Session = Depends(get_session)):
